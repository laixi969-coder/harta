...
function shellGaps(shells, tier, battlefields = []) {
  ...
  for (const b of battlefields) {
    ...
    if (!want) continue;
    if (!entries.some(([plat]) => specFor(plat)?.key === want)) {
      gaps.push(`${b}是主战场却一条外壳都没有`);
    }
  }
...
shells 是硬闸门，铺不满整份作废：信息流、朋友圈、小红书、短视频四类各至少 3 条。
先覆盖 battlefields 里的平台，再补齐剩下的类。小红书每一条都必须同时给出
cover（封面大字）、title（标题）、body（正文）三样，缺一样这条就不算数。
...
export function feedbackDigest(pack, feedback = {}) {
  ...
  for (const [group, lines] of Object.entries(pack.copies || {})) {
    const rows = lines.map((text, i) => ({ text, fb: feedback[`${pack.id}-${group}-${i}`] || "" }));
...
function refillPrompt({ customer, huntPack, prev, digest, batch, compliance }) {
  const marked = digest.groups
    .map((g) => {
      ...
      const rows = g.rows.map((r) => `- ${r.fb === "replied" ? "✓有回音" : r.fb === "dead" ? "✗没反应" : "·还没发"} ${r.text}`);
...
shells 里信息流、朋友圈、小红书、短视频四类各至少 10 条。
...
export async function generateRefill(customer, prev, feedback, batch) {
  ...
  if (digest.anyMark && !Object.keys(pack.copies).some((g) => g.startsWith("试"))) {
    throw new Error("这一批全押在已经有回音的那个方向上，没留试新方向的那一组");
  }
