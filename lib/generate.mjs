...
import { checkLength, checkPack, checkRedline } from "./check.mjs";
...
/** 能按句修的硬伤：红线词和小红书标题超限。换皮、跨平台同句这类要通篇看的，交给人。 */
...
async function selfRepair(pack, customer, onProgress) {
  ...
  onProgress(96, `自查发现 ${rows.length} 处红线或超限，正在自动修正`);
  let fixed;
  try {
    fixed = extractJson(
  ...
  for (const r of rows) {
    ...
    applyEdit(pack, r.key, now);
    applied.push({ key: r.key, where: r.where, was: r.text, now });
  }
