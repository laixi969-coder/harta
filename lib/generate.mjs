...
import { copyKey, edited, editOf } from "./pack-edits.mjs";
 ...
 * 提案是样品，3 条够看出手艺；付了钱的补给是货，一条素材跑三五天就衰减，
 * 一个账户同时要有十来条在跑，少于这个数等于第二周就断供。 */
const SHELL_FLOOR = { 快档: 3, 补给档: 10 };
...
function kindsBrief(floor) {
  ...
  const lines = Object.entries(SHELL_KINDS).map(
    ([kind, keys]) => `  - ${kind}：至少 ${floor} 条。算这一类的有 ${keys.map((k) => names[k] || k).join(" / ")}`,
  );
  return [
    ...lines,
...
function shellGaps(shells, tier, battlefields = []) {
  const gaps = [];
  ...
  for (const b of battlefields) {
    const want = specFor(b)?.key;
    if (!want) continue;
    const n = entries
      .filter(([plat]) => specFor(plat)?.key === want)
  ...
  const floor = SHELL_FLOOR[tier] || 0;
  if (!floor) return gaps;
  for (const [kind, keys] of Object.entries(SHELL_KINDS)) {
    const n = entries
      .filter(([plat]) => keys.includes(specFor(plat)?.key))
      .reduce((sum, [, items]) => sum + items.length, 0);
...
export function normalize(raw, { tier, customer }) {
  ...
  const missing = [...shellGaps(out.shells, tier, out.battlefields)];
  ...
  if (tier === "快档") {
    ...
    if (copyCount < 20) missing.push(`至少 20 条文案（只给了 ${copyCount} 条）`);
  } else {
...
const MEDICAL_VOICE = `
# 这个猎场是医疗类，第一人称有一条硬边界 ← 违反这条整份作废
《广告法》第 16 条：医疗、药品广告不得利用患者、专家名义作证明。
《互联网广告管理办法》：体验分享形式带留资或购买入口的，照样是广告，照样受这条管——
换个形式不改变它是广告这件事。

所以第一人称只写「就医过程」，不写「治好了」：
- 可以：挂号跑了几趟、哪个院区能做、报告单上哪几个数看不懂、复查时间怎么排、
  面诊该问哪三句、费用由哪几项构成、当时怎么判断该不该做、被推销了什么
- 不行：我好了 / 转阴了 / 痊愈 / 见效 / 有效果 / 指标降了 / 不疼了 /
  用了之后怎么样 / 比别家好得快 / 不用去医院 —— 任何结果性、疗效性的话，
  包括用暗示的方式说

写到「我当时是这么决定的」就停，停在动作和判断上，不写结局。
这不是把小红书写成硬广：活人感靠的是具体的犹豫和具体的钱，不靠一个好结果。
`.trim();
 ...
 * 一个月要几十条。所以这里不是"再随机出一批"，是拿上一批的反馈当输入：
 * 有回音的方向继续往下挖，没反应的方向不再出。
 *
 * 手动点。客户没反应的时候续出等于对着空气烧钱，而且素材是人拿去发的，
 * 攒着不发再多也没用。 */

/** 把上一批的反馈整理成模型看得懂的话。没点过反馈的不编，如实说还没发。 */
export function feedbackDigest(pack, feedback = {}) {
  const groups = [];
  for (const [group, lines] of Object.entries(pack.copies || {})) {
    ...
    const rows = lines.map((text, i) => {
      ...
      return {
        text: edited(pack, key, text),
        was: editOf(pack, key)?.was || "",
        fb: feedback[`${pack.id}-${group}-${i}`] || "",
      };
    });
    const replied = rows.filter((r) => r.fb === "replied");
    const dead = rows.filter((r) => r.fb === "dead");
    groups.push({ group, rows, replied, dead });
  }
  ...
  const marked = digest.groups
    .map((g) => {
      ...
      const rows = g.rows.map(
        (r) =>
          `- ${r.fb === "replied" ? "✓有回音" : r.fb === "dead" ? "✗没反应" : "·还没发"} ${r.text}` +
...
# 上一批发出去之后的实际反馈
${digest.anyMark ? marked : "销售还没有标过任何一条的反馈。"}
...
# 要出的东西，严格按这个 JSON 返回
{
  ...
  "shells": {
    "平台名": [
      { "cover": "只有小红书要，图上的大字", "title": "这个平台的标题或外层文案", "body": "只有小红书和朋友圈要" }
    ]
  },
  "breakdowns": [
...
数量要求，这是硬闸门，铺不满整份作废：
copies 分 5 到 6 组，总数不少于 25 条。其中必须有一组是「试」字开头的新方向。
shells 是硬闸门：
{SHELL_KINDS_10}
battlefields 里写的每个平台都必须有外壳。
小红书每一条都必须同时给出 cover（封面大字）、title（标题）、body（正文）三样，
缺一样这条就不算数。三样是三个不同的东西，不许互相当缩写。
breakdowns 出 3 条。

同一个钩子换几个字不算一条。二十条换皮不如十条真的不一样。
...
export async function generateRefill(customer, prev, feedback, batch) {
  ...
  const digest = feedbackDigest(prev, feedback);
  const prompt = refillPrompt({
    customer,
    huntPack,
    prev,
    digest,
    batch,
    compliance: readReference("compliance.md").slice(0, 6000),
  })
    .replace("{PLATFORM_BRIEF}", platformBrief(platformsForPack(huntPack)))
    .replace("{SHELL_KINDS_10}", kindsBrief(SHELL_FLOOR["补给档"]))
    .replace("{LANDING_RULES}", LANDING_RULES)
    .replace("{MEDICAL_VOICE}", isMedicalHunt(huntPack) ? MEDICAL_VOICE : "");
  ...
  pack.evidenceNote = digest.anyMark
    ? `第 ${batch} 批。顺着上一批标了有回音的方向出的，没反应的方向已经砍掉。`
    : `第 ${batch} 批。上一批还没有标过反馈，这一批是平铺补的，不是按效果收窄的。`;
