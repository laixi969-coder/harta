...
export function normalize(raw, { tier, customer }) {
  ...
  if (tier === "快档") {
    ...
    if (copyCount < TODAY_COPIES) missing.push(`至少 ${TODAY_COPIES} 条文案（只给了 ${copyCount} 条）`);
  } else {
...
function refillShellsPrompt({ customer, huntPack, copies, battlefields, compliance, job = "补给档", count = REFILL_COPIES }) {
...
这一份给【${job}】出平台外壳：这一批的 ${count} 条文案已经出好了，你的活是把它们翻译成各平台能直接上的成品。
文案是内核，外壳是表达形态和语气：同一条内核在三个平台不该有任何一条能互换。
外壳从这批文案里挑着包，不用每条都包，但下面每类平台的下限必须铺满。
...
function todayPrompt({ customer, huntPack, digest, compliance, battlefields, avoid }) {
...
`
    : "";
  return `
你在给已经合作的客户出本次这一批货，不是出判断报告。同一天可能已经出过别的批次，这一批仍要独立给满 50 条。
不要写缺口、提问、赛道格局、承接。这一步只出文案；平台外壳另有一次调用。
...
- 有回音时至少四分之一的条文案组名以「试」开头
- 同一个钩子换几个字不算一条
${avoidGate}- battlefieldWhy 必填，缺了整份作废
...
export async function generateTodayDrop(customer, feedback = {}, { onProgress = () => {} } = {}) {
  ...
  try {
    ...
    const raw = extractJson(await chat({ system: "你只输出 JSON。", user: prompt, maxTokens: 12000 }));
    const copies = dedupeCopies(raw.copies, prevLines);
    const copyCount = Object.values(copies).flat().length;
    if (copyCount < TODAY_COPIES) {
      throw new Error(
        avoid
          ? `去掉和最近几批重复的之后只剩 ${copyCount} 条（要 ${TODAY_COPIES} 条）。再点一次会换一批新的`
          : `至少 ${TODAY_COPIES} 条文案（只给了 ${copyCount} 条）`,
      );
    }
    onProgress(62, "50 条文案已生成，重复项检查通过");
    const battlefields = lockedFields.length ? lockedFields : (Array.isArray(raw.battlefields) ? raw.battlefields : []).slice(0, 2);
    const shellsPrompt = refillShellsPrompt({
      customer,
      huntPack,
      copies,
      ...
      .replace("{PLATFORM_BRIEF}", platformBrief(platformsForPack(huntPack)))
      .replace("{SHELL_KINDS_REFILL}", kindsBrief(SHELL_FLOOR["今日"]))
      .replace("{MEDICAL_VOICE}", isMedicalHunt(huntPack) ? MEDICAL_VOICE : "");
    onProgress(70, "正在生成各平台可直接发布的版本");
    const shellsRaw = extractJson(
      await chat({ system: "你只输出 JSON。", user: shellsPrompt, maxTokens: 12000 }),
    );
    onProgress(90, "平台版本已生成，正在检查是否齐套");
    pack = normalize(
      { ...raw, copies, battlefields, shells: shellsRaw.shells },
      { tier: "今日", customer },
    );
  } catch (err) {
