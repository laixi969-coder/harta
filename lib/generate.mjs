...
/* 存量每天就是一批货：50 条文案，四类平台各 20 条外壳。闸门和提示词同一个数。 */
...
function todayPrompt({ customer, huntPack, digest, compliance, battlefields }) {
  const fieldLine = (battlefields || []).length
    ? `主战场已经定了：${battlefields.join("、")}。不要改，这一批只出文案。`
    : `自己定主战场，最多 2 个平台。`;
  return `
你在给已经合作的客户出今天这一批货，不是出判断报告。
...
export async function generateTodayDrop(customer, feedback = {}) {
  ...
  try {
    ...
    if (copyCount < TODAY_COPIES) {
      throw new Error(`至少 ${TODAY_COPIES} 条文案（只给了 ${copyCount} 条）`);
    }
    ...
    pack = normalize(
      { ...raw, battlefields, shells: shellsRaw.shells },
      { tier: "今日", customer },
    );
  } catch (err) {
