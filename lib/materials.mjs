...
export function cleanupStaleMaterialBatches(email, now = Date.now(), ttlMs = ORPHAN_BATCH_TTL_MS) {
  ...
  try {
    ...
    referenced = new Set((workspace.customers || []).flatMap((c) => [...(c.materialBatchIds || []), c.materialBatchId]).filter(Boolean));
  } catch (error) {
    // 工作区损坏或读取失败时不能把已引用资料当作孤立文件删除。
...
/** 客户换资料或清空时删批次目录：原件、抽取正文、语音转写一起清。
 * 只删盘上目录；客户身上的引用由 workspace 那边负责改。 */
export function deleteMaterialBatch(email, batchId) {
  const id = String(batchId || "");
  if (!/^m-\d+-[a-f0-9]{8}$/.test(id)) return;
  fs.rmSync(batchDir(email, id), { recursive: true, force: true });
}
