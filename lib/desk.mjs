...
export function dropOnDate(customer, date) {
  ...
  return (customer?.drops || []).find(
    (d) => d.date === day || d.deliveredAt === day || d.createdAt === day,
  );
}
...
export function daysMissed(customer, date) {
  ...
  const last = String(customer?.lastDropAt || "");
  if (!last) return 99;
...
export function buildDesk(space, date) {
  ...
  for (const c of customers) {
    ...
    if (c.track === "存量") {
      ...
      const row = { ...c, reason: dueReason(c, day), daysMissed: daysMissed(c, day), hasToday: Boolean(todayDrop) };
      if (todayDrop && !c.job && !c.lastFail) done.push(row);
      else send.push(row);
    } else {
      const ready = hasMaterials(c) && (!hasJudgment(c) || Boolean(c.lastFail) || Boolean(c.job));
      const waiting = !hasJudgment(c) || Boolean(c.lastFail) || Boolean(c.job);
      const row = { ...c, reason: judgeReason(c), ready: hasMaterials(c) && !hasJudgment(c) };
