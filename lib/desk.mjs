...
function scheduledCount(customer, contentStates, date) {
  const ids = new Set((customer?.drops || []).map((drop) => String(drop?.id || "")).filter(Boolean));
  if (!ids.size) return 0;
  return Object.entries(contentStates || {}).filter(([key, item]) => {
    const packId = String(key).split("::", 1)[0];
    return ids.has(packId) && item?.plannedAt === date && item?.status === "selected";
  }).length;
}

function dueReason(customer, date, scheduledToday = 0) {
...
export function buildDesk(space, date) {
  ...
  for (const c of customers) {
    ...
    if (c.track === "存量") {
      ...
      const scheduledToday = scheduledCount(c, space?.contentStates, day);
      const row = deskRow(c, {
        reason: dueReason(c, day, scheduledToday),
        daysMissed: daysMissed(c, day),
        hasToday: Boolean(todayDrop),
        scheduledToday,
      });
      if (todayDrop && !scheduledToday && !c.job && !c.lastFail) done.push(row);
      else send.push(row);
    } else {
...
function hookLine({ send, judge, named, unmarked, customers }) {
  ...
  if (send.length) {
    ...
    parts.push(scheduled ? `今天还有 ${scheduled} 条内容待发布` : `存量今天还差 ${send.length} 家没出内容`);
  }
