...
/* 目标人群的描述、原话和搜索词必须排在一起；排除人群单独收在最后。
 * 否则两列布局会让右下角搜索词看起来像属于右上角的排除人群。 */
function demandBlock(d) {
  if (!d?.who) return "";
  return `<p class="meta">前三格是值得争取的目标需求；最后一格是明确排除、不建议投放的人群。</p>
    <div class="folio folio-demand">
      <div class="folio-cell is-who demand-who"><p class="k">目标人群（要截住）</p><p>${esc(d.who)}</p></div>
      <div class="folio-cell demand-say"><p class="k">目标人群会这样问</p>${paras(d.say)}</div>
      <div class="folio-cell demand-search"><p class="k">目标人群会搜索这些</p><p>${esc((d.search || []).join(" · "))}</p></div>
      <div class="folio-cell is-skip demand-skip"><p class="k">排除人群（不投）</p>${paras(d.skip)}</div>
    </div>`;
}
...
function guardrailsBlock(rows) {
  if (!rows?.length) return "";
  return `<div class="sleeves">${rows
    .map(
      (row) => `<article class="sleeve sleeve-across">
        <div class="sleeve-tab"><span>${esc(row.label)}</span><span>发布前核验</span></div>
        <div class="sleeve-body"><p>${esc(row.text)}</p></div>
      </article>`,
    )
    .join("")}</div>`;
}
...
function deliveryBlock(pack) {
  ...
  const why = pack.battlefieldWhy
    ? `<p class="meta" style="margin-top:6px">为什么是这两个：${esc(pack.battlefieldWhy)}</p>`
    : "";
  return `<div class="folio folio-3">
      <div class="folio-cell is-who"><p class="k">主战场</p><p>${esc((pack.battlefields || []).join(" + "))}</p>${why}</div>
...
export function renderPackPage(pack, customer) {
  ...
  const body = [
    section("目标需求与排除人群", demandBlock(pack.demand)),
    ...
    section("上线边界", guardrailsBlock(pack.checks?.guardrails)),
...
<head>
  ...
  <link rel="stylesheet" href="/css/app.css?v=20260831z">
</head>
<body>
  ...
  <article class="pack" id="letter">
    <header class="letterhead">
      <div>
        <img class="logo" src="/images/logo-harta.png" width="1210" height="480" alt="灵鹿线索 HARTA">
