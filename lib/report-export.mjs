...
async function exportDocx(pack, customer, tempDir) {
  ...
  await execFileAsync("zip", ["-q", "-r", output, "[Content_Types].xml", "_rels", "docProps", "word"], { cwd: tempDir, timeout: 30_000 });
...
async function exportPdf(pack, customer, tempDir) {
  ...
  return fs.readFile(path.join(tempDir, "report.pdf"));
}
...
export async function exportReport(pack, customer, format) {
  ...
  try {
    ...
    if (err?.code === "ENOENT" && err?.path === "soffice") {
      throw new Error("这台电脑没有安装 LibreOffice，暂时不能导出报告");
    }
    if (err?.code === "ENOENT" && err?.path === "zip") {
      throw new Error("这台电脑缺少 Word 导出组件，暂时不能导出报告");
    }
