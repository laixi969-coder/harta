vi.mock("./research.mjs", () => ({ buildResearch: vi.fn(async () => ({ status: "knowledge", warnings: [] })), researchBrief: () => "" }));
...
const { collectRepairs, generatePack } = await import("./generate.mjs");
...
const medicalCustomer = () => ({ ...customer(), hunt: "医药" });
...
describe("判断报告交付自查", () => {
  ...
  it("跨平台同一句和换皮在一次修复里全部改掉，只改标点的不算修好", async () => {
    const pack = {
      copies: { A: ["老房拆改先问承重墙谁签字", "老房拆改先问承重墙谁签字！"] },
      shells: {
        巨量信息流: [{ title: "进场先问这批人是谁带的" }],
        朋友圈: [{ title: "进场先问这批人是谁带的" }],
      },
    };
    repairReply = () => ({
      "朋友圈|0|title": "进场先问这批人是谁带的！",
      "A|1": "老房拆改先问承重墙谁签字。",
    });
    const missed = await collectRepairs(pack, customer());
    expect(missed.map((row) => row.key)).toEqual([]);
    repairReply = () => ({
      "朋友圈|0|title": "到现场先看防水做没做闭水试验",
      "A|1": "合同里增项超过一成，谁签字谁承担",
    });
    const fixes = await collectRepairs(pack, customer());
    expect(fixes.map((row) => row.key).sort()).toEqual(["A|1", "朋友圈|0|title"]);
  });

  it("改写后仍带着红线词的，不当作已经修好", async () => {
    repairReply = () => ({ "A|0": "我们仍然是行业第一" });
    const pack = await generatePack(customer(), { material });
    expect(pack.copies.A[0]).toContain("行业第一");
    expect(pack.origin.repairs).toBeUndefined();
  ...
  it("医疗猎场：表单收敏感信息也会被点名自修，承接伪 key 直接落到字段上", async () => {
    packReply = () => {
      const reply = JSON.parse(fastPackReply({ badLine: false }));
      reply.copies = {
        A: Array.from({ length: 8 }, (_, i) => lineOf(i + 1)),
        B: Array.from({ length: 7 }, (_, i) => lineOf(i + 20)),
      };
      reply.landing.form = ["您的HPV检测报告结果是什么"];
      return JSON.stringify(reply);
    };
    repairReply = () => ({ "landing|form|0": "您最想解决的问题是什么？" });
    // 医药是医疗猎场：表单收「检测报告结果」要被敏感闸门点名
    const pack = await generatePack(medicalCustomer(), { material });
    expect(pack.landing.form[0]).toBe("您最想解决的问题是什么？");
    expect(pack.checks.sensitive).toEqual([]);
    expect(pack.origin.repairs.map((r) => r.key)).toContain("landing|form|0");
    // 自查的提示词点名了敏感收集，让模型知道往哪改
    const repairPrompt = chatCalls.find((u) => u.includes("改好的句子"));
    expect(repairPrompt).toContain("医疗健康敏感信息");
  });
});
