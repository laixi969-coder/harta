...
import { deliveryIssues, platformKind, titleCount } from '../js/platform-content.js';
...
/* 疗效词和「这句话是不是在承诺」不是一回事。
 * 科普纠偏必须点出「转阴」「HPV转阴」才能告诉读者别信；
 * 若见到这两个字就硬拦，纠偏稿永远过不了，自动修复也删不掉主题词，下载就卡死。
 * 硬拦的是肯定式承诺。否定、提问、所谓误区、读者自己的着急，降到「看一眼」。
 * 处方药、靶向药、购药入口不走这套：前面加「不要」也不能出现。 */
const CLAIM_BREAK = /[，。！？；\n]/;
const CLAIM_REVERSAL = /不要错过|别错过|不是不|并非不|并不是不/;
const CLAIM_PROMISE_TAIL = /(?:可以|能够|能让|能保证|帮你|帮助你|助你|保证|确保|一定|肯定|必定|实现|达到|快速|包你|三个月|几个月|数月)$/;
const CLAIM_REJECTION = /不是|不算|没有|没能|并不能|无法|不能|不可|不得|不要|别信|别买|别急|先别|别再|并非|而非|绝非|哪有|谈不上|不存在|不写|不说|禁止|所谓|误区|谣言|辟谣|纠偏|不保证|不能保证|不承诺|不得承诺|没法|不会|不等于|与否|靠谱吗|靠不/;
const CLAIM_QUESTION = /为什么|怎么还|还没|是不是|能不能|靠不靠谱|靠谱吗/;

function clauseRejects(text) {
  return CLAIM_REJECTION.test(text) || /别(?!的)/.test(text);
}

function promiseGoverns(before) {
  const window = before.slice(-18);
  const matched = window.match(CLAIM_PROMISE_TAIL);
  if (!matched) return false;
  const head = window.slice(0, window.length - matched[0].length);
  return !/(?:不|没|别|非|无|勿|所谓|误区)$/.test(head);
}

/** 这一处是在承诺结果，还是在否定、提问、点名误区。true = 硬拦。 */
function isAffirmativeMedicalClaim(text, word, at) {
  let start = 0;
  for (let i = at - 1; i >= 0; i -= 1) {
    if (CLAIM_BREAK.test(text[i])) { start = i + 1; break; }
  }
  let end = text.length;
  for (let i = at + word.length; i < text.length; i += 1) {
    if (CLAIM_BREAK.test(text[i])) { end = i; break; }
  }
  const before = text.slice(start, at);
  const after = text.slice(at + word.length, end);
  const clause = before + word + after;
  if (CLAIM_REVERSAL.test(before)) return true;
  if (promiseGoverns(before)) return true;
  if (/^(?:了|啦)/.test(after) && /(?:已经|都)$/.test(before) && !clauseRejects(before)) return true;
  if (/^(?:率|方案|特效)/.test(after) && !clauseRejects(before)) return true;
  if (clauseRejects(before + after)) return false;
  const question = CLAIM_QUESTION.test(clause) || /[?？]/.test(text[end] || "");
  if (question && !/(?:我们|本机构|方案|产品|药物|这套)/.test(clause)) return false;
  if (/(?:想|希望|着急|追求|盼)/.test(before.slice(-8)) && !/(?:我们|本机构|方案|产品|药物|这套)/.test(clause)) return false;
  return true;
}
...
function occurrences(hay, needle) {
  ...
  for (;;) {
    ...
    if (found < 0) return at;
    at.push(found);
    from = found + needle.length;
  }
}

/** 疗效词：原文和去空格后各扫一遍，只要有一处是承诺就拦。全是纠偏则放行。 */
function medicalClaimHits(body, words) {
  const compact = compactMedical(body);
  const found = [];
  for (const word of words) {
    const places = occurrences(body, word).map((at) => [body, word, at]);
    const compactWord = compactMedical(word);
    if (compactWord && (compact !== body || compactWord !== word)) {
      for (const at of occurrences(compact, compactWord)) places.push([compact, compactWord, at]);
    }
    if (places.some(([hay, needle, at]) => isAffirmativeMedicalClaim(hay, needle, at))) found.push(word);
  }
  return found;
}

/** 拦死的那一档：确凿的疗效承诺，以及无论如何都不能出现的处方药和购药入口。 */
export function checkRedline(text, hunt) {
  const body = String(text);
  const general = hits(body, generalRedlines());
  const fromHunt = huntRedlines(hunt);
  if (!isMedicalHunt(hunt)) return [...new Set([...general, ...hits(body, fromHunt)])];
  const claims = [...new Set([
    ...MEDICAL_HARD.filter((word) => !MEDICAL_NO_PUBLIC.includes(word)),
    ...fromHunt.filter((word) => !MEDICAL_NO_PUBLIC.includes(word)),
  ])];
  return [
    ...new Set([
      ...general,
      ...medicalClaimHits(body, claims),
      ...hits(body, MEDICAL_NO_PUBLIC),
      // 处方药、靶向药、购药入口即使放在否定句里也不进成品。
...
export function checkLength(text, platform, fieldKey) {
  ...
  const n = platformKind(platform) === 'xiaohongshu' && field.key === 'title' ? titleCount(text) : charCount(text);
...
export function checkPack(pack, hunt) {
  // Natural-content delivery checks the actual copy; it does not issue institutional approvals.
  const guardrails = pack.origin?.mode === 'organic' ? [] : huntGuardrails(hunt);
  const redline = [];
  const sensitive = [];
  const watch = [];
  const hints = [];
  ...
  for (const [platform, items] of Object.entries(sent.shells)) {
    for (const [index, raw] of (items || []).entries()) {
      ...
      // 诊断中的样例用于说明方向；完整发布字段是内容交付的要求。
      const isReport = pack.tier && pack.tier !== '今日' && pack.origin?.mode !== 'organic';
      const issues = isReport ? [] : deliveryIssues(platform, item, { complete: pack.origin?.deliveryVersion >= 2 });
      if (issues.length) {
        const texts = Object.values(item).filter(Boolean);
        for (const text of texts.length ? texts : ['']) structure.push({ where:`${platform} · 第${index+1}篇`, text, level:'hard', ...(texts.length ? {} : {scope:'batch'}), platform, index, code:'platform-structure', why:issues.map(x=>x.why).join('；') });
      }
  ...
  const quality = [...structure, ...judgeQuality({ copies: sent.copies, shells: sent.shells })];
  return {
    guardrails,
    redline,
    sensitive,
    ...
    length,
    quality,
    note: "标「发布风险」「发不出去」「质量过不了」的必须改。科普里点出「转阴」来否定它，不算承诺，只在「看一眼」里提醒。「看一眼」和「不像这个平台」由你定。字数建议值以各平台当前发布页面为准。",
    checkedAt: today(),
  };
}
