...
describe("未建档的资料批次不会无限占盘", () => {
  ...
  it('追加前的旧批次仍被引用，工作区无法解析时不清理任何批次', () => {
    const id=workspaceId(EMAIL), root=path.join(sandbox,'data','materials',id);
    const file=path.join(sandbox,'data','workspaces',`${id}.json`);
    const older='m-501-abcde001', latest='m-502-abcde002';
    makeBatch(root,older,'2026-08-01T00:00:00Z');
    makeBatch(root,latest,'2026-08-01T00:00:00Z');
    fs.writeFileSync(file,JSON.stringify({customers:[{materialBatchId:latest,materialBatchIds:[older,latest]}]}));
    cleanupStaleMaterialBatches(EMAIL,Date.parse('2026-09-01T12:00:00Z'));
    expect(fs.existsSync(path.join(root,older))).toBe(true);
    fs.writeFileSync(file,'{broken');
    cleanupStaleMaterialBatches(EMAIL,Date.parse('2026-09-02T12:00:00Z'));
    expect(fs.existsSync(path.join(root,older))).toBe(true);
