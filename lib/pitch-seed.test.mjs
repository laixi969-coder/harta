...
describe("served-client seed", () => {
  it("keeps three served brands each with a 15-copy 快档", () => {
    ...
    for (const c of space.customers) {
      expect(c.track).toBe("存量");
      expect(c.packs).toHaveLength(1);
      expect(c.drops).toHaveLength(1);
      expect(c.drops[0].tier).toBe("今日");
      expect(copyCount(c.drops[0])).toBe(15);
      expect(c.drops[0].gaps).toEqual([]);
      expect(c.drops[0].landing).toBeNull();
      expect(c.drops[0].evidenceNote).toMatch(/演示今日内容/);
