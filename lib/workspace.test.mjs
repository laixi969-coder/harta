...
let releaseBoards;
let releaseRepairs;
const repairsMock = vi.fn(() => new Promise(resolve => { releaseRepairs = resolve; }));
...
vi.mock("./generate.mjs", () => ({
  collectRepairs: (...args) => repairsMock(...args),
  sentenceRepairCount: () => 0,
...
describe("工作区", () => {
  ...
  it('异步修复保留等待期间的业务修改，并跳过人工已改的同一文案', async () => {
    seedWorkspace({customer:{track:'拓新'}});
    const pending=fixPack(EMAIL,'p1');
    updateCustomerBusiness(EMAIL,'c1',{pitch:'最新业务',city:'重庆',salesMaterial:'新增合作范围'});
    editLine(EMAIL,'p1','A 组|0','人工定稿');
    releaseRepairs([{key:'A 组|0',text:'第一条',now:'模型旧修复'},{key:'A 组|1',text:'第二条',now:'模型修复'}]);
    const result=await pending;
    const customer=result.workspace.customers[0];
    expect(customer.salesMaterial).toBe('新增合作范围');
    expect(customer.packs[0].edits['A 组|0'].now).toBe('人工定稿');
    expect(customer.packs[0].edits['A 组|1'].now).toBe('模型修复');
    expect(result.fixed).toBe(1);
  });

  it('工作区写入失败时保留旧数据，不留下半截文件', () => {
    seedWorkspace({customer:{track:'拓新',stage:'new'}});
    const rename=vi.spyOn(fs,'renameSync').mockImplementation(()=>{throw Error('磁盘失败');});
    try { expect(()=>setCustomerStage(EMAIL,'c1','following')).toThrow('磁盘失败'); }
    finally { rename.mockRestore(); }
    expect(readWorkspace(EMAIL).customers[0].stage).toBe('new');
    expect(fs.readdirSync(path.dirname(WS_FILE())).some(name=>name.endsWith('.tmp'))).toBe(false);
  });

  it('旧阶段接口也必须遵守任务锁，重复设置拓新不会把在跟客户退回新客户', () => {
    seedWorkspace({customer:{track:'拓新',stage:'following',job:{id:'busy'}}});
    expect(setTrack(EMAIL,'c1','存量').error).toContain('任务');
    seedWorkspace({customer:{track:'拓新',stage:'following'}});
    expect(setTrack(EMAIL,'c1','拓新').workspace.customers[0].stage).toBe('following');
  });

  it('诊断接口不能意外触发合作客户内容任务', () => {
    seedWorkspace({customer:{track:'存量'}});
    expect(repack(EMAIL,'c1').error).toContain('内容生成入口');
    expect(readWorkspace(EMAIL).customers[0].job).toBeFalsy();
  });

  it('同一毫秒创建两位客户仍有独立 ID', () => {
    seedWorkspace();
    const clock = vi.spyOn(Date,'now').mockReturnValue(1800000000000);
    try {
      const input={name:'客户',hunt:'家装',pitch:'旧改',track:'存量'};
      const a=addCustomer(EMAIL,input).workspace.usingId;
      const b=addCustomer(EMAIL,input).workspace.usingId;
      expect(a).not.toBe(b);
      expect(readWorkspace(EMAIL).customers.filter(c=>[a,b].includes(c.id))).toHaveLength(2);
    } finally { clock.mockRestore(); }
  });

  it('超时后可以重试，旧任务晚返回不能覆盖或解锁新任务', async () => {
    seedWorkspace({customer:{id:'late-job'}});
    expect(upgradeToFull(EMAIL,'p1').error).toBeUndefined();
    const oldResolve=releaseBoards;
    const space=readWorkspace(EMAIL);
    space.customers[0].job.startedAt=Date.now()-26*60*1000;
    fs.writeFileSync(WS_FILE(),JSON.stringify(space));
    sweepStaleJobs(EMAIL);
    expect(upgradeToFull(EMAIL,'p1').error).toBeUndefined();
    const newResolve=releaseBoards;
    const current=readWorkspace(EMAIL).customers[0].job.id;
    oldResolve([{title:'旧结果',shots:[]}]);
    await new Promise(resolve=>setTimeout(resolve,0));
    expect(readWorkspace(EMAIL).customers[0].job.id).toBe(current);
    expect(readWorkspace(EMAIL).customers[0].packs[0].boards).toBeUndefined();
    expect(upgradeToFull(EMAIL,'p1').error).toBeTruthy();
    newResolve([{title:'新结果',shots:[]}]);
    await new Promise(resolve=>setTimeout(resolve,0));
    expect(readWorkspace(EMAIL).customers[0].packs[0].boards[0].title).toBe('新结果');
  });

  it('客户阶段转换保留原报告与资料，合作后不自动生成内容', () => {
    seedWorkspace({ customer: { track: '拓新', stage: 'new', salesMaterial: '客户原话' } });
    expect(setCustomerStage(EMAIL, 'c1', 'following').workspace.customers[0].track).toBe('拓新');
    const result = setCustomerStage(EMAIL, 'c1', 'cooperating').workspace.customers[0];
    expect(result.track).toBe('存量');
    expect(result.packs[0].id).toBe('p1');
    expect(result.salesMaterial).toBe('客户原话');
    expect(result.job).toBeFalsy();
    expect(result.stageHistory.map(row => row.to)).toEqual(['following', 'cooperating']);
    expect(setCustomerStage(EMAIL, 'c1', 'invalid').error).toBeTruthy();
  });

  it('直接录入合作客户可以先完善业务资料，不启动内容任务', () => {
    seedWorkspace();
    const result = addCustomer(EMAIL, { name: '合作客户', hunt: '家装', pitch: '旧改', track: '存量', startContent: false });
    const created = result.workspace.customers.find(c => c.name === '合作客户');
    expect(created.stage).toBe('cooperating');
    expect(created.job).toBeFalsy();
    const updated = updateCustomerBusiness(EMAIL, created.id, { pitch:'旧房翻新', city:'重庆', salesMaterial:'已确认合作范围' });
    expect(updated.workspace.customers.find(c => c.id === created.id)).toMatchObject({ pitch:'旧房翻新', city:'重庆', salesMaterial:'已确认合作范围' });
  });

  it('承接表单收 HPV 型别时可以改成普通问题，改完不再挡住下载', () => {
    seedWorkspace({
      customer: { hunt: '医药' },
      pack: { landing: { firstScreen: '先列要问医生的问题', form: ['填写HPV型别'], reward: '问诊问题清单' } },
    });
    expect(editLine(EMAIL, 'p1', 'landing|form|0', '您最想先了解哪一步？').error).toBeUndefined();
    const pack = readWorkspace(EMAIL).customers[0].packs[0];
    expect(pack.landing.form[0]).toBe('您最想先了解哪一步？');
    expect(pack.checks.sensitive).toEqual([]);
  });

  it('编辑小红书标题超20字被拒绝，原稿保持不变',()=>{
    seedWorkspace({pack:{shells:{小红书:[{title:'原来标题',cover:'原来封面',body:'正文'}]}}});
    expect(editLine(EMAIL,'p1','小红书|0|title','HPV'+'字'.repeat(18)).error).toContain('21字');
    expect(readWorkspace(EMAIL).customers[0].packs[0].edits).toBeUndefined();
    expect(editLine(EMAIL,'p1','小红书|0|title','字'.repeat(20)).error).toBeUndefined();
  });

  it('旧内容补齐缺失封面后仍可选择、排期和导出',()=>{
    seedWorkspace({pack:{tier:'今日',shells:{小红书:[{title:'预约前问清楚',body:'正文内容'}]}}});
    expect(editLine(EMAIL,'p1','小红书|0|cover','先问再预约').error).toBeUndefined();
    expect(setContentState(EMAIL,{packId:'p1',keys:['p1::小红书|0|cover'],status:'selected'}).error).toBeUndefined();
    expect(readWorkspace(EMAIL).customers[0].packs[0].checks.quality.filter(x=>x.code==='platform-structure')).toHaveLength(0);
  });
  ...
  it("平台询价按客户 ID 归因，三字段只挂一个反馈并留存实际内容", () => {
    const space = seedWorkspace({ customer: { track: "存量", drops: [{ id: "d1", tier: "今日", copies: {}, shells: { 小红书: [{ cover: "封面", title: "标题", body: "正文" }] }, edits: { "小红书|0|body": { now: "改后正文" } } }] } });
    space.customers.push({ id: "same-name", name: "测试客户", hunt: "口腔", packs: [] });
    fs.writeFileSync(WS_FILE(), JSON.stringify(space));
    expect(addLedger(EMAIL, { client: "测试客户", result: "问了价", talk: "多少钱" }).error).toMatch(/同名/);
    const result = addLedger(EMAIL, { customerId: "c1", client: "伪造名称", hunt: "口腔", result: "问了价", talk: "多少钱", line: "d1-平台-小红书-0" });
    expect(result.marked).toBe(true);
    expect(result.workspace.ledger[0]).toMatchObject({ customerId: "c1", client: "测试客户", hunt: "家装", platform: "小红书", line: "d1-平台-小红书-0", packId: "d1" });
    expect(result.workspace.ledger[0].content).toContain("改后正文");
    expect(Object.keys(result.workspace.feedback)).toEqual(["d1-平台-小红书-0"]);
    for (const field of ["cover", "title", "body"]) expect(result.workspace.contentStates[`d1::小红书|0|${field}`].status).toBe("published");
  });

  it("平台反馈保留排期，不接受陌生行或伪造反馈值", () => {
    seedWorkspace({ customer: { drops: [{ id: "d1", tier: "今日", copies: {}, shells: { 朋友圈: ["正文"] } }] } });
    setContentState(EMAIL, { packId: "d1", keys: ["d1::朋友圈|0|title"], status: "selected", plannedAt: "2026-09-01" });
    const after = setFeedback(EMAIL, "d1-平台-朋友圈-0", "dead");
    expect(after.contentStates["d1::朋友圈|0|title"]).toMatchObject({ status: "published", plannedAt: "2026-09-01" });
    expect(setFeedback(EMAIL, "other-A-0", "replied").feedback["other-A-0"]).toBeUndefined();
    expect(setFeedback(EMAIL, "d1-平台-朋友圈-0", "bogus").feedback["d1-平台-朋友圈-0"]).toBe("dead");
    const published = setContentState(EMAIL, { packId: "d1", keys: ["d1::朋友圈|0|title"], status: "published" });
    expect(published.workspace.contentStates["d1::朋友圈|0|title"].plannedAt).toBe("2026-09-01");
    expect(addLedger(EMAIL, { customerId: "other", result: "成交", talk: "成了" }).error).toBeTruthy();
  });
  ...
  it("台账日期用本地口径", () => {
    ...
    expect(space.ledger[0].date).toBe(`${local.getFullYear()}-${localMD}`);
  });
  ...
  it("台账记问价并挂了行，那条自动算有回音", () => {
    ...
    expect(out.workspace.contentStates["p1::A 组|0"].status).toBe("published");
  });

  it("内容批次可以批量标记选用、计划和发布，且不接受别人的行", () => {
    seedWorkspace({
      customer: {
        track: "存量",
        packs: [],
        drops: [{ id: "d1", tier: "今日", copies: { "A 组": ["第一条", "第二条"] }, shells: {} }],
      },
    });
    const selected = setContentState(EMAIL, {
      packId: "d1",
      keys: ["d1::A 组|0"],
      status: "selected",
      plannedAt: "2026-09-08",
    });
    expect(selected.updated).toBe(1);
    expect(selected.workspace.contentStates["d1::A 组|0"]).toMatchObject({ status: "selected", plannedAt: "2026-09-08" });
    const published = setContentState(EMAIL, { packId: "d1", keys: ["d1::A 组|0"], status: "published" });
    expect(published.workspace.contentStates["d1::A 组|0"].publishedAt).toBeTruthy();
    expect(setContentState(EMAIL, { packId: "d1", keys: ["other::A 组|0"], status: "selected" }).error).toMatch(/不属于/);
  });
...
describe("资料包裹的删除与重建", () => {
  ...
  it('补充资料保留旧附件，重复提交不重复追加，明确清空时释放所有批次', () => {
    seedBatch('m-1-aaaa0001', '旧'); seedBatch('m-2-bbbb0002', '新');
    seedCustomer('c1', 'm-1-aaaa0001');
    const result = replaceCustomerMaterials(EMAIL, 'c1', 'm-2-bbbb0002', { append: true });
    expect(result.workspace.customers[0].materials).toHaveLength(2);
    expect(new Set(result.workspace.customers[0].materials.map(source=>source.id)).size).toBe(2);
    expect(result.workspace.customers[0].materialAnalysis.overview).toContain('旧总览');
    expect(fs.existsSync(path.join(MAT_DIR(), 'm-1-aaaa0001'))).toBe(true);
    expect(replaceCustomerMaterials(EMAIL, 'c1', 'm-2-bbbb0002', { append:true }).workspace.customers[0].materials).toHaveLength(2);
    clearCustomerMaterials(EMAIL, 'c1');
    expect(fs.existsSync(path.join(MAT_DIR(), 'm-1-aaaa0001'))).toBe(false);
    expect(fs.existsSync(path.join(MAT_DIR(), 'm-2-bbbb0002'))).toBe(false);
  });
