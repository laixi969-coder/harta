import { acquisitionBrief } from "./generate.mjs";
import { describe, expect, it } from "vitest";
import { dedupeCopies, exploreShortfall, extractJson, feedbackDigest, normalize, validateHuntPack } from "./generate.mjs";
import {
  checkLength,
  checkPack,
  checkRedline,
...
describe("交付前检查", () => {
  ...
  it("医药的确定性禁区直接硬拦：HPV 结果承诺和处方药公众推广不能只提醒", () => {
    ...
    expect(checkRedline("HPV转阴", "医药").length).toBeGreaterThan(0);
    expect(checkRedline("临床转阴率高", "医药").length).toBeGreaterThan(0);
    expect(checkRedline("医生说已经转阴了", "医药").length).toBeGreaterThan(0);
    expect(checkRedline("不要错过转阴机会", "医药").length).toBeGreaterThan(0);
    expect(checkRedline("我们能让你转阴", "医药").length).toBeGreaterThan(0);
  });

  it("科普纠偏点出转阴不是承诺，不能卡住整份报告", () => {
    const texts = [
      "查出来HPV阳性的人，先别急着花大价钱买所谓的转阴特效药",
      "为什么还没转阴？\n避开这3个误区",
      "有些姐妹查出HPV后，特别着急想让它“转阴”。目前并没有所谓的“神药”能保证所有人都转阴。转阴与否不是唯一的指标。",
      "别把HPV转阴当成治疗结果",
    ];
    for (const text of texts) {
      expect(checkRedline(text, "医药"), text).toEqual([]);
      expect(checkWatch(text, "医药"), text).toContain("转阴");
    }
    const pack = {
      tier: "快档",
      copies: { "A组_科普纠偏": [texts[2]] },
      shells: {
        小红书: [{ title: "避开三个误区", cover: texts[1], body: texts[2] }],
        视频号: [{ cover: texts[3], title: "别把网上的说法当结果", body: "随访要问医生什么" }],
      },
    };
    const checked = checkPack(pack, "医药");
    expect(checked.redline).toEqual([]);
    expect(hasHardBlock(checked)).toBe(false);
    expect(checked.watch.length).toBeGreaterThan(0);
  });
  ...
  it("小红书标题保守计数，英文数字不折半", () => {
    expect(checkLength("字".repeat(21), "小红书", "title").n).toBe(21);
    expect(checkLength("a".repeat(20), "小红书", "title")).toBeNull();
    expect(checkLength("a".repeat(21), "小红书", "title").level).toBe('hard');
  });
});
...
describe("再出一批不重复", () => {
  it("和最近几批一字不差的（含只差空格的）全部去掉", () => {
    const prev = ["老房报价单有 4 个坑", "同样90平，两家差9万"];
    const out = dedupeCopies(
      {
        "A 避坑": ["老房报价单有4个坑", "老房报价单有 4 个坑", "全新的一条钩子"],
        "B 报价": ["同样 90 平，两家差 9 万", "另一条也是新的"],
      },
      prev,
    );
    expect(out["A 避坑"]).toEqual(["全新的一条钩子"]);
    expect(out["B 报价"]).toEqual(["另一条也是新的"]);
  });

  it("这一批内部互相重复的只留第一条，空掉的组整个拿掉", () => {
    const out = dedupeCopies(
      {
        "A 避坑": ["同一句话", "同一句话", "  同一句话  "],
        "B 报价": ["只差空格算重复"],
    ...
    expect(out["A 避坑"]).toEqual(["同一句话"]);
    expect(out["B 报价"]).toEqual(["只差空格算重复"]);
    expect(out["C 报价"]).toBeUndefined();
  });

  it("今天的档缺主战场依据照样拦，给了就放行", () => {
    const base = {
      title: "t",
      gate: "g",
      battlefields: ["小红书"],
      copies: { 组: Array.from({ length: 50 }, (_, i) => `第${i}条不一样的文案`) },
      shells: {
        小红书: Array.from({ length: 20 }, (_, i) => ({ cover: `封${i}`, title: `题${i}`, body: `文${i}` })),
        信息流: Array.from({ length: 20 }, (_, i) => `流${i}`),
        朋友圈: Array.from({ length: 20 }, (_, i) => `圈${i}`),
        短视频: Array.from({ length: 20 }, (_, i) => `频${i}`),
      },
    };
    expect(() => normalize({ ...base }, { tier: "今日", customer: { id: "t" } })).toThrow(/主战场的依据/);
    expect(() => normalize({ ...base, battlefieldWhy: "客群在小红书密度最高" }, { tier: "今日", customer: { id: "t" } })).not.toThrow();
  });
});


describe("平台反馈进入下一批", () => {
  it("包括真实平台版本的改稿及回音，不编造未标记平台的结果", () => {
    const p = { id: "platform", copies: {}, shells: { 小红书: [{ title: "标题", body: "正文" }], 朋友圈: ["没有反馈"] }, edits: { "小红书|0|body": { now: "销售改稿" } } };
    const digest = feedbackDigest(p, { "platform-平台-小红书-0": "replied" });
    expect(digest.anyMark).toBe(true);
    expect(digest.groups).toHaveLength(1);
    expect(digest.groups[0].group).toBe("平台反馈 · 小红书");
    expect(digest.groups[0].replied[0].text).toContain("销售改稿");
  });
});


it("长销售补充不挤掉附件依据", () => {
  const brief = acquisitionBrief({ salesMaterial: "补".repeat(12000), sourceMaterial: "附件中的真实服务范围" });
  expect(brief).toContain("附件中的真实服务范围");
  expect(brief).toContain("补".repeat(100));
});
