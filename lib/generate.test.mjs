...
function fastPack(over = {}) {
  return {
    ...
    landing: {
      way: "表单留资",
      firstScreen: "先看你手里那张报价单漏了哪三项",
      form: ["手机号（要回电）", "房子面积（决定报价口径）"],
      reward: "一份按项拆开的报价对照表",
      leak: "留资后隔天才回电，意向早被同行接走",
    },
...
describe("出档闸门", () => {
  ...
  it("侦察档必须有提问和赛道格局，没有就不许出", () => {
    const recon = {
      ...
      landing: { way: "表单留资", firstScreen: "先看报价单漏了哪三项", reward: "一份报价对照表" },
  ...
  it("侦察档不铺四类，但主战场每个要满 3 条：3 条文案各有各的成品", () => {
    const recon = (shells) => ({
      title: "t", gate: "g", battlefields: ["巨量信息流", "小红书"],
      demand: { who: "w", say: ["a"], search: ["b"], skip: ["c"] },
      copies: { 试探: ["1", "2", "3"] }, shells,
      landing: { way: "表单留资", firstScreen: "先看这个", reward: "一份清单" },
