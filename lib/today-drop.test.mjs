...
function replyFor(user) {
  ...
  if (user.includes("你在给已经合作的客户出本次这批货里「试新方向」")) {
...
const { generateInventoryDrop: generateTodayDrop } = await import("./generate.mjs");
...
describe("历史批量库存兼容管线", () => {
  it("真实客户资料进入文案和平台版本，平台回音及来源随批次留底", async () => {
    const c = customer();
    c.id = "c1";
    c.salesMaterial = "只服务本地老房，先沟通预算再预约量房。";
    c.drops = [{ id: "old", copies: { A: ["历史文案"] }, shells: { 小红书: [{ title: "历史标题", body: "历史正文" }] } }];
    const pack = await generateTodayDrop(c, { "old-平台-小红书-0": "replied" }, { material: "附件真实资料：服务范围限本地，按需求现场核价。", ledger: [{ customerId: "c1", platform: "小红书", packId: "old", result: "问了价" }, { customerId: "other", platform: "小红书", packId: "old", result: "成交" }] });
    const prompts = chatCalls.filter((u) => u.includes("第 1 段文案") || u.includes("这一类平台的外壳"));
    expect(prompts.length).toBeGreaterThanOrEqual(5);
    for (const prompt of prompts) {
      expect(prompt).toContain("服务范围限本地");
      expect(prompt).toContain("先沟通预算再预约量房");
      expect(prompt).toContain("平台反馈 · 小红书");
      expect(prompt).toContain("询价 1 次，成交 0 次");
    }
    expect(pack.origin.feedbackSources).toEqual(["old"]);
    expect(pack.origin.feedbackSummary).toEqual([{ group: "平台反馈 · 小红书", replied: 1, dead: 0 }]);
  });
