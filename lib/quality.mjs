...
function walkCopies(copies, fn) {
  for (const [group, lines] of Object.entries(copies || {})) {
    (Array.isArray(lines) ? lines : []).forEach((text, i) => fn(`${group} · ${i + 1}`, text, copyKey(group, i)));
  }
}
...
function judgeSlogans(copies, shells, issues) {
  walkCopies(copies, (where, text, key) => {
...
function judgeClones(copies, issues) {
  const rows = [];
  walkCopies(copies, (where, text, key) => rows.push({ where, text, key }));
    ...
    for (let j = 0; j < i; j += 1) {
      if (alike(rows[i].text, rows[j].text)) {
        ...
        push(issues, {
          ...
          key: rows[i].key,
          kind: "clone",
          counterpart: rows[j].text,
          counterpartWhere: rows[j].where,
          level: "hard",
