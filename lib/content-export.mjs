import { shellFeedbackKey } from "../js/acquisition.js";
...
import { CONTENT_FIELDS, FIELD_LABELS, deliveryFields } from '../js/platform-content.js';
    ...
    (items || []).forEach((raw, i) => {
      ...
      for (const field of CONTENT_FIELDS) {
        ...
        shells.push({
          ...
          status: state.status,
          statusLabel: contentStatusLabel(state.status),
          group: pack.origin?.mode === 'organic' ? Object.keys(pack.copies || {})[i] || '' : '',
          platform,
          number: i + 1,
          field: deliveryFields(platform).find(f=>f.key===field)?.label || FIELD_LABELS[field] || field,
          text: item[field],
          edited: edit ? "是" : "否",
          original: edit?.was || "",
          feedback: feedback[shellFeedbackKey(pack.id, platform, i)] === "replied" ? "有回音" : feedback[shellFeedbackKey(pack.id, platform, i)] === "dead" ? "没反应" : "",
...
export function contentExportData({ pack, customer, contentStates = {}, feedback = {}, options = {} }) {
  ...
  const kind = ["copies", "shells", "all"].includes(options.kind) ? options.kind : organic ? "shells" : "all";
  ...
  if (scope === "group") rows = rows.filter((row) => (organic || row.kind === "copy") && row.group === String(options.group || ""));
...
function shellSheetRows(rows) {
  return [
    ["批次", "生成日期", "计划日期", "发布日期", "状态", "平台", "序号", "字段", "最终内容", "是否修改", "原稿", "整条反馈", "合规"],
    ...rows.map((row) => [row.batch, row.generatedAt, row.plannedAt, row.publishedAt, row.statusLabel, row.platform, row.number, row.field, row.text, row.edited, row.original, row.feedback, row.risk || "可发布"]),
  ];
}
