...
export function addToWhitelist(email) {
  ...
  if (data.whitelist.includes(clean)) return { error: "已经在白名单里" };
...
export function removeFromWhitelist(email) {
  const clean = normalizeEmail(email);
  if (clean === ADMIN_EMAIL) return { error: "不能把超级管理员移出白名单" };
  const data = readAuth();
...
export function loginOrBootstrap(email, password) {
  ...
  if (!user) return { error: "账号不存在，白名单用户请先注册" };
  if (!user.passwordHash) {
    if (user.role !== "admin") return { error: "此账号尚未开通" };
...
export function register(email, password) {
  ...
  if (clean === ADMIN_EMAIL) return { error: "超级管理员请直接登录" };
  if (!isWhitelisted(clean)) return { error: "不在白名单，请联系超级管理员" };
