...
export function isPrivateAddress(ip) {
  ...
  // URL 会把 IPv4 映射地址规范化为十六进制，不能只检测点分十进制。
  ...
  // 只接受全球单播地址；拒绝链路本地、多播、未指定和转换地址。
  return first < 0x2000 || first > 0x3fff || low.startsWith('2001:db8:');
}
...
export async function assertPublicUrl(raw) {
  ...
  if (url.username || url.password) throw new Error("网址不能包含账号密码");
...
async function readLimitedText(res) {
  ...
  while (total < MAX_BYTES) {
    ...
    const kept = chunk.subarray(0, MAX_BYTES - total);
    chunks.push(kept);
    total += kept.length;
    if (kept.length < chunk.length || total >= MAX_BYTES) {
      await reader.cancel();
      break;
    }
  }
  ...
  try {
    ...
    for (let hop = 0; hop < 5; hop += 1) {
      ...
      if (res.status >= 300 && res.status < 400 && location) {
        if (hop === 4) return { ok: false, why: "网页跳转次数太多" };
        try {
          current = await assertPublicUrl(new URL(location, current).href);
        } catch (err) {
          return { ok: false, why: `网页跳转被拦下：${err.message}` };
        }
        continue;
      }
      break;
    }
    if (!res) return { ok: false, why: "网页没有返回内容" };
    if (!res.ok) return { ok: false, why: `打开返回 ${res.status}` };
    const type = res.headers.get("content-type") || "";
    if (!/text\/html|text\/plain|application\/xhtml/i.test(type)) {
      return { ok: false, why: "这个地址不是网页" };
    }
    const text = htmlToText(await readLimitedText(res)).slice(0, MAX_CHARS);
    if (text.length < 80) return { ok: false, why: "页面抓回来几乎没有文字，多半是要登录或全靠脚本渲染" };
    return { ok: true, url: current.href, text };
  } catch (err) {
