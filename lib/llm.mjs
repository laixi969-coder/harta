...
const DOCS = {
  ...
  volces: "https://console.volcengine.com/ark",
...
/* 没写过 enabled 的老配置是旧版自动选择留下的，也按全关迁移。 */
...
/* 内置渠道的种子。国内三家（百炼、方舟、DeepSeek）排在最前：用的人多，
 * 不该让他们翻到最后一屏才找到自己那家。 */
const FRONT_BUILTINS = ["dashscope", "volces", "deepseek"];
const BUILTIN_SEEDS = {
  dashscope: { name: "阿里云百炼", kind: "openai", baseUrl: "https://dashscope.aliyuncs.com/compatible-mode/v1" },
  volces: { name: "火山引擎方舟", kind: "openai", baseUrl: "https://ark.cn-beijing.volces.com/api/v3" },
  deepseek: { name: "DeepSeek", kind: "openai", baseUrl: "https://api.deepseek.com/v1" },
  xai: { name: "xAI", kind: "openai", baseUrl: "https://api.x.ai/v1" },
  ...
  custom: { name: "自定义兼容接口", kind: "openai", baseUrl: "" },
};

function seedProvider(id) {
  return { id, ...EMPTY_PROVIDER(), ...BUILTIN_SEEDS[id] };
}

function defaults() {
  return {
    active: "xai",
    providers: Object.fromEntries(Object.keys(BUILTIN_SEEDS).map((id) => [id, seedProvider(id)])),
  };
}

/* 老配置落盘时还没有国内三家。读到缺的就地补上并顶到最前；不主动写盘，
 * 之后任何一次改动落盘，新顺序自然就存住了。 */
function ensureFrontBuiltins(data) {
  if (!data.providers) data.providers = {};
  const missing = FRONT_BUILTINS.filter((id) => !data.providers[id]);
  if (missing.length) {
    for (const id of missing) data.providers[id] = seedProvider(id);
    const rest = Object.keys(data.providers).filter((id) => !FRONT_BUILTINS.includes(id));
    data.providers = Object.fromEntries(
      [...FRONT_BUILTINS, ...rest].map((id) => [id, data.providers[id]]),
    );
  }
  for (const p of Object.values(data.providers)) {
    normalizeProviderModelSelection(p);
  }
  return data;
}
...
export function readLlm() {
  ...
  return ensureFrontBuiltins(JSON.parse(fs.readFileSync(file, "utf8")));
}
...
export function publicLlm() {
  ...
  for (const [id, p] of Object.entries(data.providers)) {
    providers[id] = {
      ...
      models: sortModelsNewestFirst(p.models || [], p.modelCreatedAt || {}),
 ...
 * 内置那几个是固定的，删不掉；自己加的随时能删。 */
const BUILTIN = [...FRONT_BUILTINS, "xai", "openai", "anthropic", "custom"];

export function addCustomProvider(name) {
  const field = textField(name, "渠道名字", INPUT_LIMITS.providerName, { required: true });
  if (field.error) throw new Error(field.error === "渠道名字不能为空" ? "给这个接口起个名字，好认" : field.error);
  const clean = field.text;
  const data = readLlm();
  let n = 2;
...
export function removeProvider(id) {
  ...
  if (data.active === id) {
    ...
    data.active = ids.find((x) => providerReadiness(data.providers[x]).ready) || ids[0] || "xai";
  }
  writeLlm(data);
  return publicLlm();
}
...
export function saveProvider(id, patch) {
  ...
  const wasReady = providerReadiness(p).ready;
  const keyField = textField(patch.apiKey, "API Key", INPUT_LIMITS.apiKey);
  const baseField = textField(patch.baseUrl, "接口地址", INPUT_LIMITS.baseUrl);
  const nameField = textField(patch.name, "渠道名字", INPUT_LIMITS.providerName);
  const modelField = textField(patch.model, "模型名", INPUT_LIMITS.modelName);
  if (patch.apiKey !== undefined && keyField.error) throw new Error(keyField.error);
  if (patch.baseUrl !== undefined && baseField.error) throw new Error(baseField.error);
  if (patch.name !== undefined && nameField.error) throw new Error(nameField.error);
  if (patch.model !== undefined && modelField.error) throw new Error(modelField.error);
  const nextKey = patch.apiKey ? keyField.text : p.apiKey;
  const nextBase = patch.baseUrl !== undefined ? baseField.text.replace(/\/$/, "") : p.baseUrl;
  const nextModel = patch.model !== undefined ? modelField.text : p.model;
  ...
  if (patch.name !== undefined && !BUILTIN.includes(id)) {
    p.name = nameField.text || p.name;
  }
  const nextReadiness = providerReadiness(p);
  if (data.active === id && wasReady && !nextReadiness.ready) {
    throw new Error(`正在使用的渠道不能清空${nextReadiness.missing.join("、")}；请先切换到其他渠道`);
  }
  if (patch.active === true) {
    if (!nextReadiness.ready) throw new Error(`这个渠道还不能使用，请先补齐${nextReadiness.missing.join("、")}`);
    data.active = id;
  }
  writeLlm(data);
  return publicLlm();
...
export function setActive(id) {
  ...
  if (!readiness.ready) throw new Error(`这个渠道还不能使用，请先补齐${readiness.missing.join("、")}`);
  data.active = id;
  writeLlm(data);
  return publicLlm();
}
...
export function setModelEnabled(id, model, on) {
  ...
  if (name.length > INPUT_LIMITS.modelName) throw new Error(`模型名最多 ${INPUT_LIMITS.modelName} 个字符`);
  const enabledSet = new Set(enabledList(p));
  if (!on && data.active === id && p.model === name && enabledSet.size === 1) {
    throw new Error("正在使用的模型不能直接关到一个不剩；请先选择另一个模型或切换渠道");
  }
  if (on) {
    if (!(p.models || []).includes(name)) p.models.unshift(name);
    enabledSet.add(name);
  } else {
...
export function addModel(id, model) {
  ...
  p.models = sortModelsNewestFirst([...(p.models || []), name], p.modelCreatedAt || {});
  (p.manualModels ||= []).push(name);
  p.enabled = enabledList(p);
  writeLlm(data);
  return publicLlm();
}
...
export async function syncModels(id) {
  ...
  if (p.kind === "anthropic") {
    ...
    records = (got.json.data || []).filter((m) => m?.id);
  } else {
    const got = await fetchJson(`${base}/models`, {
  ...
  const names = records.map((m) => m.id);
  p.modelCreatedAt ||= {};
  for (const record of records) {
    const createdAt = modelCreatedAt(record);
    if (createdAt) p.modelCreatedAt[record.id] = createdAt;
  }
  /* 手动加过的（同步列表里不出现的火山 endpoint 这类）留在清单里，不能一次同步就冲掉 */
  const all = sortModelsNewestFirst([...names, ...(p.manualModels || [])], p.modelCreatedAt);
  const prevEnabled = new Set(enabledList(p));
  const prevList = new Set(p.models || []);
  p.models = all;
  /* 老名字保持用户自己的选择；新同步到的模型一律不替用户打开。 */
  p.enabled = selectedModelsAfterSync(all, prevList, prevEnabled);
...
/* 结构化内容生成使用非思考模式；只给文档确认支持的渠道和模型发送扩展参数。
 * 百炼：https://help.aliyun.com/zh/model-studio/deep-thinking */
export function thinkingOff(base, model = "") {
  let host;
  try { host = new URL(base).hostname; } catch { return {}; }
  if (host === "volces.com" || host.endsWith(".volces.com")) return { thinking: { type: "disabled" } };
  if (["dashscope.aliyuncs.com", "dashscope-intl.aliyuncs.com", "dashscope-us.aliyuncs.com"].includes(host) && /^qwen3\.[5-8]-(?:plus|flash|max)(?:-|$)/.test(model) && !model.includes("thinking")) return { enable_thinking: false };
  return {};
}

/** 出档用的通用对话调用。默认返回纯文本；资料审计可要求同时返回模型信息。 */
export async function chat({ system, user, maxTokens = 8000, returnMeta = false }) {
  const data = readLlm();
  ...
  try {
    ...
    if (p.kind === "anthropic") {
      ...
      return returnMeta ? { text, providerId: p.id || data.active, providerName: p.name, model: p.model } : text;
    }
...
async function chatWithProviderImages(p, { system, user, images = [], maxTokens = 3000 }) {
  const base = (p.baseUrl || "").replace(/\/$/, "");
  const kept = selectVisualEvidence(images, 8);
  const ctl = AbortController ? new AbortController() : null;
  const timer = setTimeout(() => ctl?.abort(), CHAT_TIMEOUT_MS);
  try {
    let res;
...
export async function chatWithImages(args) {
  const data = readLlm();
  const providerIds = new Map(Object.entries(data.providers || {}).map(([id, p]) => [p, id]));
  const selectedImages = selectVisualEvidence(args.images, 8);
  const request = { ...args, images: selectedImages };
  ...
  for (const p of candidates) {
    try {
      const text = await chatWithProviderImages(p, request);
      // 视觉请求成功不代表结果真的可用。例如资料梳理要求 JSON，有些模型会只回
      // 一段说明。让上层带一个轻量校验器进来；校验不过也继续试下一个视觉模型。
      if (typeof args.validate === "function") args.validate(text);
      return args.returnMeta
        ? {
            text,
            providerId: p.id || providerIds.get(p) || "",
            providerName: p.name,
            model: p.model,
            visualsAvailable: Array.isArray(args.images) ? args.images.length : 0,
            visualsSent: selectedImages.length,
          }
        : text;
    } catch (err) {
      errors.push(`${p.name || p.id} / ${p.model || "未选模型"}：${err.message || "视觉调用失败"}`);
    }
  }
