...
export async function syncModels(id) {
  ...
  const all = [...new Set([...names, ...(p.manualModels || [])])];
  const prevEnabled = new Set(enabledList(p));
  const prevList = new Set(p.models || []);
  /* 这次新冒出来的排最上面，同步完一眼看到带来了什么；
   * 老名单照上次的顺序跟在后面，上次顶上来的不会又被压回字母序里 */
  p.models = [...all.filter((m) => !prevList.has(m)), ...(p.models || []).filter((m) => all.includes(m))];
