import { cachedKeywordBasis } from './keyword-basis-cache.mjs';
import { CUSTOMER_STAGES, customerStage } from '../js/customer-stage.js';
...
import { collectRepairs, generateBoards, generatePack, generateTodayDrop, sentenceRepairCount } from "./generate.mjs";
...
import { CONTENT_FIELDS, deliveryFields, platformKind, titleCount } from '../js/platform-content.js';

const dir = () => path.join(process.cwd(), "data", "workspaces");
const materialIds = c => [...new Set([...(c.materialBatchIds || []), c.materialBatchId].filter(Boolean))];
const customerMaterialText = (email, c) => materialIds(c).map(id => materialTextForPack(readMaterialBatch(email, id))).filter(Boolean).join('\n\n【补充资料】\n');
...
export function writeWorkspace(email, data) {
  ...
  try {
    fs.writeFileSync(temporary, JSON.stringify(rest, null, 2) + "\n");
...
export function publicWorkspace(space) {
  return { ...space, customers: space.customers.map(c => ({...c, keywordOpportunities: cachedKeywordBasis(c)})), desk: buildDesk(space) };
}
...
function startJob(email, customerId, kind, make, apply) {
  ...
  (async () => {
    try {
      ...
      if (running.get(key) === jobId) running.delete(key);
    }
  })();
...
export function sweepStaleJobs(email) {
  ...
  for (const c of space.customers || []) {
    ...
    if (dead) {
      if (running.get(jobKey(email, c.id)) === c.job.id) running.delete(jobKey(email, c.id));
...
export function addCustomer(email, input) {
  ...
  const customer = {
    ...
    track,
    stage: track === '存量' ? 'cooperating' : 'new',
    materials: batch
  ...
  if (track === "存量") return { workspace: space };
...
export function setGrowthDirection(email, customerId, direction) {
  const field = textField(direction, "调整方向", 2000);
  if (field.error) return { error: field.error };
  const space = readWorkspace(email);
  const customer = (space.customers || []).find((c) => c.id === customerId);
  if (!customer) return { error: "没有这个客户" };
  customer.growthDirection = field.text;
  customer.directionUpdatedAt = new Date().toISOString();
  writeWorkspace(email, space);
...
export function setTrack(email, customerId, track) {
  ...
  const stage = track === '存量' ? 'cooperating' : customerStage(c) === 'following' ? 'following' : 'new';
  return setCustomerStage(email, customerId, stage);
}

export function setCustomerStage(email, customerId, stage) {
  if (!Object.hasOwn(CUSTOMER_STAGES, stage)) return { error: '请选择有效的客户阶段' };
  const space = readWorkspace(email);
  const customer = space.customers.find(c => c.id === customerId);
  if (!customer) return { error: '没有这个客户' };
  if (customer.job) return { error: '当前任务完成后再调整客户阶段' };
  const previous = customerStage(customer);
  customer.stage = stage;
  customer.track = stage === 'cooperating' ? '存量' : '拓新';
  if (previous !== stage) {
    customer.stageHistory = [...(customer.stageHistory || []), { from: previous, to: stage, at: new Date().toISOString() }];
  }
  writeWorkspace(email, space);
  return { workspace: space };
}

export function updateCustomerBusiness(email, customerId, input) {
  const fields = {
    pitch: textField(input.pitch, '业务简介', INPUT_LIMITS.pitch, { required: true }),
    city: textField(input.city, '城市', INPUT_LIMITS.city),
    salesMaterial: textField(input.salesMaterial, '业务补充', INPUT_LIMITS.salesMaterial),
  };
  ...
  const customer = space.customers.find(c => c.id === customerId);
  if (!customer) return { error: '没有这个客户' };
  for (const [key, field] of Object.entries(fields)) customer[key] = field.text;
  writeWorkspace(email, space);
  return { workspace: space };
}
...
function freeMaterialBatch(space, email, batchId, keepBatchId = "") {
  ...
  const stillUsed = (space.customers || []).some((c) => materialIds(c).includes(batchId));
...
export function clearCustomerMaterials(email, customerId) {
  ...
  writeWorkspace(email, space);
  old.forEach(id => freeMaterialBatch(space, email, id));
  return { workspace: space };
}

export function replaceCustomerMaterials(email, customerId, batchId, { append = false } = {}) {
  const space = readWorkspace(email);
  const c = (space.customers || []).find((x) => x.id === customerId);
  if (!c) return { error: "没有这个客户" };
  ...
  const old = materialIds(c);
  if (append && old.includes(batch.id)) return { workspace: space };
  // 每批的源文件编号都从 s1 开始，合并时必须带批次，避免摘要串到另一份文件。
  const qualify = (row, batchId) => ({...row, id: String(row.id).includes(':') ? row.id : `${batchId}:${row.id}`});
  const prior = append && c.materialAnalysis ? {...c.materialAnalysis, sourceNotes:(c.materialAnalysis.sourceNotes || []).map(row=>qualify(row,c.materialBatchId))} : null;
  const analysis = batch.analysis ? {...batch.analysis, sourceNotes:(batch.analysis.sourceNotes || []).map(row=>qualify(row,batch.id))} : null;
  c.materials = [...(append ? (c.materials || []).map(row=>qualify(row,c.materialBatchId)) : []), ...(batch.sources || []).map(({ text, storedName, ...source }) => qualify(source,batch.id))];
  c.materialAnalysis = analysis;
  if (prior) {
    c.materialAnalysis ||= {};
    c.materialAnalysis.overview = [prior.overview, batch.analysis?.overview].filter(Boolean).join('\n\n');
    for (const key of ['offer','audience','proof','risks','missing','sourceNotes']) c.materialAnalysis[key] = [...(prior[key] || []), ...(analysis?.[key] || [])];
  }
  c.materialReady = (append && c.materialReady) || materialTextForPack(batch).trim().length >= 60;
  c.materialBatchId = batch.id;
  c.materialBatchIds = append ? [...old, batch.id] : [batch.id];
  writeWorkspace(email, space);
  if (!append) old.forEach(id => freeMaterialBatch(space, email, id, batch.id));
  return { workspace: space };
}
...
export function repack(email, customerId, input = {}) {
  ...
  if (c.track === "存量") return { error: '合作客户请使用内容生成入口，诊断接口不会启动内容任务' };
  ...
  return startJob(email, customerId, "重出", (cur, report) => {
    const archived = customerMaterialText(email, cur);
...
export function dropToday(email, customerId) {
  ...
  return startJob(
    ...
    customerId,
    "出今日",
    (cur, report) => generateTodayDrop(cur, feedback, { material: customerMaterialText(email, cur), ledger: space.ledger, onProgress: report }),
    applyTodayDrop,
  );
}
  ...
  for (const [platform, items] of Object.entries(pack?.shells || {})) {
    for (let i = 0; i < (items || []).length; i += 1) {
      ...
      for (const field of CONTENT_FIELDS) {
        if (item[field] || pack.edits?.[shellKey(platform,i,field)]?.now) keys.add(contentItemKey(pack.id, shellKey(platform, i, field)));
      }
    }
  }
  return keys;
...
export function editLine(email, packId, key, text) {
  ...
  for (const c of space.customers || []) {
    ...
    if(parts.length===3 && parts[2]==='title' && platformKind(parts[0])==='xiaohongshu' && titleCount(now)>20) return {error:`小红书标题最多20字，当前${titleCount(now)}字（英文、数字、标点也计入）；请缩短后保存`};
    if (parts[0] === "landing") {
      if (now !== was) applyLandingEdit(pack, key, now);
...
function originalOf(pack, key) {
  if (String(key).startsWith("landing|")) {
    const landing = pack.landing || {};
    const [, field, idxRaw] = String(key).split("|");
    const idx = Number(idxRaw);
    if (field === "firstScreen") return landing.firstScreen ?? null;
    if (field === "reward") return landing.reward ?? null;
    if (field === "form") return Array.isArray(landing.form) && Number.isInteger(idx) && landing.form[idx] != null ? landing.form[idx] : null;
    if (field === "open") return landing.firstTouch?.open?.[idx]?.say ?? null;
    if (field === "pushback") return landing.firstTouch?.pushback?.[idx]?.reply ?? null;
    if (field === "rewardOutline") return Array.isArray(landing.rewardOutline) && landing.rewardOutline[idx] != null ? landing.rewardOutline[idx] : null;
    return null;
  }
  for (const [group, lines] of Object.entries(pack.copies || {})) {
    for (let i = 0; i < (lines || []).length; i += 1) {
  ...
  for (const [plat, items] of Object.entries(pack.shells || {})) {
    for (let i = 0; i < (items || []).length; i += 1) {
      ...
      for (const f of CONTENT_FIELDS) {
        if (shellKey(plat, i, f) === key && (item[f] || deliveryFields(plat).some(field=>field.key===f))) return item[f] || '';
      }
    }
  }
  return null;
 ...
 * 返回 { workspace, fixed, repairable }。
 * fixed 是这次改成的句数。repairable 是改完后仍能按句修的硬伤；
 * 为 0 时剩下的是换皮、跨平台同句，或者已经没有硬伤。
 */
export async function fixPack(email, packId) {
  ...
  for (const c of space.customers || []) {
    ...
    const latest = readWorkspace(email);
    const currentCustomer = latest.customers.find(row => row.id === c.id);
    const current = [...(currentCustomer?.packs || []), ...(currentCustomer?.drops || [])].find(row => row.id === packId);
    if (!current) return { error: '这份报告已删除，修复结果未写入' };
    if (!fixes.length) return { workspace: latest, fixed: 0, repairable: sentenceRepairCount(current, currentCustomer.hunt) };
    current.edits = current.edits || {};
    const landingUnchanged = JSON.stringify(pack.landing) === JSON.stringify(current.landing);
    const applied = [];
    for (const f of fixes) {
      if (String(f.key).startsWith("landing|")) {
        if (!landingUnchanged) continue;
        applyLandingEdit(current, f.key, f.now);
      } else {
        if (JSON.stringify(pack.edits?.[f.key]) !== JSON.stringify(current.edits?.[f.key]) || originalOf(pack, f.key) !== originalOf(current, f.key)) continue;
        const was = originalOf(current, f.key) ?? f.text;
        if (was !== f.now) current.edits[f.key] = { was, now: f.now, at: today() };
      }
      applied.push({ key: f.key, where: f.where, was: f.text, now: f.now });
    }
    current.checks = checkPack(current, currentCustomer.hunt);
    current.origin = { ...(current.origin || {}), repairs: [...(current.origin?.repairs || []), ...applied] };
    writeWorkspace(email, latest);
    return { workspace: latest, fixed: applied.length, repairable: sentenceRepairCount(current, currentCustomer.hunt) };
  }
  return { error: "找不到这份档" };
}

/** 只在异步解析完成后读写当前客户，保留同时发生的方向/内容变更。 */
export function saveKeywordLibrary(email, customerId, batch) {
  const space = readWorkspace(email);
  const customer = space.customers.find(c => c.id === customerId);
  if (!customer) return { error: '没有这个客户，或不属于当前账号' };
  const libraries = customer.keywordLibraries || [];
  if (libraries.length >= 10 || libraries.reduce((n, x) => n + x.items.length, 0) + batch.items.length > 10000) return { error: '每个业务最多10批、合计10000条关键词，请移除不再使用的批次后导入' };
  customer.keywordLibraries = [...libraries, batch];
  customer.keywordsUpdatedAt = new Date().toISOString();
  writeWorkspace(email, space);
  return { workspace: space };
}
export function removeKeywordLibrary(email, customerId, batchId) {
  const space = readWorkspace(email);
  const customer = space.customers.find(c => c.id === customerId);
  if (!customer) return { error: '没有这个客户，或不属于当前账号' };
  if (!(customer.keywordLibraries || []).some(b => b.id === batchId)) return { error: '没有这批关键词' };
  customer.keywordLibraries = customer.keywordLibraries.filter(b => b.id !== batchId);
  customer.keywordsUpdatedAt = new Date().toISOString();
  writeWorkspace(email, space);
  return { workspace: space };
}
