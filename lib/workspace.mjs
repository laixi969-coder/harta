...
import { materialTextForPack, readMaterialBatch } from "./materials.mjs";
...
export function addCustomer(email, input) {
  ...
  const material = [materialTextForPack(batch), String(input.material || "").trim()]
    .filter(Boolean)
    .join("\n\n【销售补充】\n");
...
export function repack(email, customerId, input = {}) {
  const extra = String(input.material || "").trim();
  return startJob(email, customerId, "重出", (c) => {
    const archived = materialTextForPack(readMaterialBatch(email, c.materialBatchId));
    const material = [archived, extra].filter(Boolean).join("\n\n【销售补充】\n");
    return generatePack(c, { link: c.link, material });
  });
}
