...
export function addCustomer(email, input) {
  ...
  if (track === "存量") return input.startContent === false ? { workspace: space } : dropToday(email, customer.id);
...
export function setTrack(email, customerId, track) {
  ...
  c.track = track;
  c.stage = track === '存量' ? 'cooperating' : 'new';
  writeWorkspace(email, space);
  return { workspace: space };
}
...
export function replaceCustomerMaterials(email, customerId, batchId, { append = false } = {}) {
  ...
  c.materials = [...(append ? c.materials || [] : []), ...(batch.sources || []).map(({ text, storedName, ...source }) => source)];
  const prior = append ? c.materialAnalysis : null;
  c.materialAnalysis = batch.analysis ? { ...batch.analysis } : null;
  if (prior) {
    c.materialAnalysis ||= {};
    c.materialAnalysis.overview = [prior.overview, batch.analysis?.overview].filter(Boolean).join('\n\n');
    for (const key of ['offer','audience','proof','risks','missing','sourceNotes']) c.materialAnalysis[key] = [...(prior[key] || []), ...(batch.analysis?.[key] || [])];
  }
