...
export function dropToday(email, customerId) {
  ...
  return startJob(
    ...
    customerId,
    "出今日",
    (cur, report) => generateTodayDrop(cur, feedback, { onProgress: report }),
    applyTodayDrop,
  );
}
...
export function setFeedback(email, key, value) {
  ...
  const owned = feedbackContentKey(space, key);
  if (owned && (value === "replied" || value === "dead")) {
    space.contentStates = space.contentStates || {};
    const previous = space.contentStates[owned] || {};
    space.contentStates[owned] = {
      ...previous,
      status: "published",
...
function feedbackContentKey(space, feedbackKey) {
  for (const c of space.customers || []) {
    for (const pack of [...(c.drops || []), ...(c.packs || [])]) {
      for (const [group, lines] of Object.entries(pack.copies || {})) {
        for (let i = 0; i < (lines || []).length; i += 1) {
          if (`${pack.id}-${group}-${i}` === feedbackKey) {
            return contentItemKey(pack.id, copyKey(group, i));
          }
        }
      }
    }
...
export function addLedger(email, row) {
  ...
  if (marked) {
    ...
    const key = feedbackContentKey(space, marked);
    if (key) {
      space.contentStates = space.contentStates || {};
      const previous = space.contentStates[key] || {};
      space.contentStates[key] = {
        ...previous,
        status: "published",
        publishedAt: previous.publishedAt || new Date().toISOString(),
      };
